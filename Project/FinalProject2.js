...
function draw() { 
	...
	fireworks.push(new Firework(random(1000),random(100)));
  background(0,0);
  for (var i = fireworks.length - 1; i >= 0; i--) {//display the firework
    fireworks[i].display();
    fireworks[i].update();
    if (fireworks[i].end) {
...
function drawcanv2() {	
    ...
    var speed = map(mouseX-560, 60, 140, 0, 4);
...
class Firework {
  ...
  for (var i = 0; i < random(100, 200); i++) {
    this.particles[i] = new Particle(x, y, this.c, this.m);
  }
  }
  display () {//draw the firework
    for (var i = this.particles.length - 1; i >= 0; i--) {
      this.particles[i].display();
    }
  }

  update() {//update the firework
    for (var i = this.particles.length - 1; i >= 0; i--) {
...
class  Particle{
  constructor(x, y, c, mode, d) {
  this.loc = createVector(x, y);
  this.cen = createVector(x, y);
  this.g = 0.2;
  this.c = c;
  this.d = d;
  this.lifeSpan = 100.0;
  this.deadSpeed = random(1, 5);
  this.speed = createVector(random(-6, 6), random(-10, 3));
  if (mode)
    this.c = color(random(255), random(255), random(255));
  }
  display() {//draw each point
    stroke(this.c);
    strokeWeight(5);
    point(this.loc.x, this.loc.y);
  }

  update() {//move each point
    this.speed.y += this.g;
